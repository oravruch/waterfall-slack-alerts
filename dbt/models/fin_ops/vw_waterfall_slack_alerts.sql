{{ config(
    materialized='view',
    database='prod_incubation',
    schema='fin_ops',
    alias='vw_waterfall_slack_alerts'
) }}

-- Copy of sql/vw_waterfall_slack_alerts.sql body (without CREATE VIEW wrapper).
-- Deploy: dbt run --select vw_waterfall_slack_alerts --target prod
